...
echo \#include \"converterApp.h\">> $outputPath$outputFile
echo>> $outputPath$outputFile
echo const wxString ConverterApp::versionString = _T\(\"$gitTag\"\)\;>> $outputPath$outputFile
echo const wxString ConverterApp::gitHash = _T\(\"$gitHash\"\)\;>> $outputPath$outputFile
